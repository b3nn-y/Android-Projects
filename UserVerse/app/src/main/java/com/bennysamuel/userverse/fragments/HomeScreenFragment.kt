...
//            fetchLocation()
    ...
    fun sortArrayAndGenRecyclerView(query: String?){
        if (query == null || query == ""){
            ...
            val tempList: ArrayList<DualUser> = viewModel.apiDataList
            val sortedUsersByStartingLetters: ArrayList<User> = ArrayList()
            val sortedUsersByContains: ArrayList<User> = ArrayList()
            for (i in tempList){
